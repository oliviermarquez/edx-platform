...
class TestGenerateCourseBlocks(ModuleStoreTestCase):
    ...
    def test_no_dags(self, mock_log):
        self.command.handle(dags=True, all_courses=True)
        self._assert_message_presence_in_logs('DAG alert', mock_log, expected_presence=False)

    @patch('lms.djangoapps.course_blocks.management.commands.generate_course_blocks.log')
    def test_with_dags(self, mock_log):
        with self.store.branch_setting(ModuleStoreEnum.Branch.draft_preferred):
            course = self.courses[0]
            item1 = ItemFactory.create(parent=course)
            ...
            self.store.publish(course.location, ModuleStoreEnum.UserID.mgmt_command)

        self.command.handle(dags=True, all_courses=True)
        self._assert_message_presence_in_logs('DAG alert', mock_log)
